...
| 12 | 🟢 | Feature | Added "Filter by Bookclub" toggle button to notes nav | New button in the notes toolbar filters notes to only bookclub works and sorts them in chronological reading-plan order (by year then order). Uses `.tabOn` inverted-color style when active. Wired in `state.js`, `notes.js`, `wire-ui.js`, and `index.html`. |
