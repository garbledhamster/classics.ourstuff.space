...
This script:
1. Loads all books from project.json (398 entries with src URLs)
2. Generates a markdown file with a matrix showing:
   - ✅ Available on Project Gutenberg or partner sites (with URL)
   - ⚠️ Uncertain/Need verification
   - ⛔ Not available (if manually marked)

Supported Project Gutenberg sites:
   - gutenberg.org (US)
...
def build_source_mapping(project_data):
    """
    ...
    for book in project_data:
        title = book.get('title', '').strip()
        author = book.get('author', '')
        source_url = book.get('src', '')
...
def generate_markdown(books, sources):
    ...
    md.append("- To update sources, add or modify `src` entries in `project.json`")
