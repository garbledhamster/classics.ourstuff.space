#!/usr/bin/env python3
"""
Script to generate GUTENBERG_SOURCES.md from library.json

This script:
1. Loads all books from library.json (398 entries)
2. Uses sourceUrl field from library.json for books available on multiple Project Gutenberg sites:
   - gutenberg.org (US)
...
def build_source_mapping(books):
    """
    Build a mapping of book titles to their source URLs from library.json
...
def generate_markdown(books, sources):
    ...
    md.append("## Notes")
    md.append("")
    md.append("- This file is automatically generated from `library.json`")
    md.append("- To update sources, add or modify `sourceUrl` entries in `library.json`")
...
def main():
    ...
    books = load_json_file('library.json')
    print(f"  Loaded {len(books)} books")
    
    print("Building source mapping from library.json...")
