...
(()=> {
  ...
  async function loadGlossaryTerms(){
    ...
    try {
      ...
      glossaryState.terms = getRawTermsPayload(data).map(normalizeTerm).sort((a,b) => {
        return a.letter.localeCompare(b.letter) || a.term.localeCompare(b.term, undefined, { sensitivity: "base" });
      });
  ...
  function availableLetters(){
    return Array.from(new Set(glossaryState.terms.map(t => t.letter))).sort();
  }
    ...
    if (!$("#glossaryView")) {
      ...
      section.innerHTML = `
        ...
        <section class="glossaryControls" aria-label="glossary filters">
          <div class="control">
            <div class="label">Search terms, references, and ideas</div>
            <input id="glossaryQ" class="input" type="search" placeholder="e.g., Justice, Soul, Matter…" autocomplete="search">
          </div>
          <div class="control">
            <div class="label">Letter</div>
            <select id="glossaryLetterSel" class="select"><option value="">All Letters</option></select>
          </div>
        </section>
        <div class="glossaryLayout">
          <aside class="glossaryLetters" aria-label="Syntopicon letters">
            <div class="glossaryLettersTitle">Letters</div>
            <div id="glossaryLetterGrid" class="glossaryLetterGrid"></div>
          </aside>
  ...
  function buildGlossaryModal(){
    ...
    modal.innerHTML = `
      <button id="glossaryPrevBtn" class="glossaryModalNav glossaryModalPrev" type="button" aria-label="Previous glossary panel">
        <svg xmlns="http://www.w3.org/2000/svg" width="30" height="30" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><polyline points="15 18 9 12 15 6"></polyline></svg>
      </button>
      <div class="glossaryModalMain">
        <div class="glossaryModalHeader">
          <div>
            <h2 id="glossaryModalTitle" class="glossaryModalTitle"></h2>
            <div id="glossaryModalSub" class="glossaryModalSub"></div>
          </div>
          <button id="glossaryCloseBtn" class="glossaryCloseBtn" type="button" aria-label="Close glossary term">×</button>
        </div>
        <div id="glossaryModalBody" class="glossaryModalBody"></div>
      </div>
      <button id="glossaryNextBtn" class="glossaryModalNav glossaryModalNext" type="button" aria-label="Next glossary panel">
        <svg xmlns="http://www.w3.org/2000/svg" width="30" height="30" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><polyline points="9 18 15 12 9 6"></polyline></svg>
      </button>
    `;
  ...
  function wireGlossaryEvents(){
    ...
    $("#glossaryQ")?.addEventListener("input", e => {
      glossaryState.query = e.target.value;
      applyGlossaryFilters();
      renderGlossary();
    });

    $("#glossaryLetterSel")?.addEventListener("change", e => {
      glossaryState.activeLetter = e.target.value;
      applyGlossaryFilters();
      renderGlossary();
    });
  ...
  function renderGlossary(){
    ...
    const letterGrid = $("#glossaryLetterGrid");
    const letterSel = $("#glossaryLetterSel");
    ...
    letterGrid.innerHTML = [`<button class="btn glossaryLetterBtn${!glossaryState.activeLetter ? " active" : ""}" data-letter="" type="button">All</button>`]
      .concat(letters.map(letter => `<button class="btn glossaryLetterBtn${letter === glossaryState.activeLetter ? " active" : ""}" data-letter="${escapeHtml(letter)}" type="button">${escapeHtml(letter)}</button>`))
      .join("");

    if (glossaryState.loading) {
      index.innerHTML = `<div class="glossaryEmpty">Loading Syntopicon terms…</div>`;
      return;
    }

    if (glossaryState.error) {
      index.innerHTML = `
        <div class="glossaryEmpty">
          <strong>Glossary data not loaded.</strong><br>
          Add <span class="mono">syntopicon_terms.json</span> to the site root. Error: ${escapeHtml(glossaryState.error)}
        </div>`;
      return;
    }

    if (!glossaryState.filteredTerms.length) {
      index.innerHTML = `<div class="glossaryEmpty">No glossary terms match the current filters.</div>`;
      return;
    }
    ...
    index.innerHTML = grouped.map(([letter, terms]) => `
      <section class="glossarySection" id="glossary-letter-${escapeHtml(letter)}">
        ...
        <div class="glossaryTermList">
          ${terms.map(term => `
            <button class="glossaryTermBtn" type="button" data-term-id="${escapeHtml(term.id)}">
              <span>${escapeHtml(term.term)}</span>
              ${term.qualifier ? `<span class="glossaryTermQualifier"> (${escapeHtml(term.qualifier)})</span>` : ""}
            </button>
          `).join("")}
        </div>
      </section>
  ...
  function closeTermModal(){
    ...
    if (!["loginModal", "signupModal", "searchSettingsModal", "timerModal"].some(id => $("#" + id)?.classList.contains("open"))) {
  ...
  function renderSyntopiconPage(term){
    renderPageShell(term, "Syntopicon References", `
      ...
      <div class="glossaryBlock">
        <div class="glossaryBlockTitle">See</div>
        <div class="glossaryRefText">${escapeHtml(term.see || "No primary references listed.")}</div>
      </div>
      <div class="glossaryBlock">
  ...
  function relatedLibraryItems(term){
    ...
    return works.map(work => {
      ...
      for (const idea of ideaNorms){
        if (idea && haystack.includes(idea)) score += 10;
      }
      ...
      ${related.length ? `<div class="glossaryRelatedList">
        ${related.map(({ work }) => `
          <div class="glossaryRelatedItem">
            <div class="glossaryRelatedTitle">${escapeHtml(work.title)}</div>
            <div class="glossaryRelatedMeta">${escapeHtml(work.author || "Unknown author")}${work.greatIdeas?.length ? ` • ${escapeHtml(work.greatIdeas.slice(0, 6).join(", "))}` : ""}</div>
          </div>
        `).join("")}
